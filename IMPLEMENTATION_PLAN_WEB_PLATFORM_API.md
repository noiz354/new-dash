...
## §33 — Implementation Status (wave 5: tsc-debt cleanup + backlog TASK-22/27/28/29/30)

**Wave 5 status (committed):**

- **[DONE] TASK-22** — CSP berubah dari REPORT-ONLY → ENFORCE di `next.config.mjs` (directive identik, key diganti, komentar diperbarui). Migrasi inline handler print → `components/print/PrintButton.tsx` (client component) dipakai oleh 3 halaman print: `app/work-orders/[id]/print/page.tsx`, `app/permits/[id]/print/page.tsx`, `app/badges/[id]/print/page.tsx`. Zero-dep; `report-uri` tetap dipantau. (Acceptance plan: print regression hijau + violations=0 — belum dieksekusi manual.)
- **[DONE] TASK-27** — Web Push VAPID opt-in. Server: `lib/push/push-service.ts` (VAPID env `WEBPUSH_PUBLIC_VAPID_KEY`/`WEBPUSH_PRIVATE_VAPID_KEY`, fallback ephemeral-dev-only + warn), subscription persistence di tabel `push_subscriptions` (migrasi `db/migrations/0001_sleepy_lilith.sql`), pruning 404/410 otomatis, endpoint `/api/push/subscribe` (GET public key / POST subscribe / DELETE unsubscribe). Wiring: P1 SLA eskalasi (`sendVendorEscalation` di `lib/services/notification-service.ts`) memicu push bila `reason` memuat `P1` atau `slaMinutesRemaining ≤ 30`. Client: `components/pwa/PushOptIn.tsx` (feature-detected `navigator.serviceWorker` + `PushManager`, hidden kalau tidak tersedia) dan SW handlers `push`/`notificationclick` di `public/sw.js` (deep link ke `/work-orders/<woNumber>`). Note test integrasi 2-browser belum dieksekusi (manual).
- **[DONE] TASK-28** — Passkeys/WebAuthn. Server: `lib/auth/webauthn.ts` (@simplewebauthn/server), `pendingRegister`/`pendingLogin` maps (in-memory, single-process dev; produksi perlu Redis), registration role-limited via permission `settings.manage` (technisi/viewer di-forbiden server-side), TOTP gate tetap aktif setelah passkey (bila `user.totpSecret`). API: `app/api/auth/passkeys/register/route.ts` (GET options/POST verify) + `app/api/auth/passkeys/login/route.ts` (POST options public / PUT verify public / GET list / DELETE revoke). UI: `components/auth/PasskeySettings.tsx` (list + tambah + revoke; public-key credential di-browser) dan tombol Passkey pada `components/auth/LoginForm.tsx` (`startAuthentication`). Tabel `webauthn_credentials` (migrasi `0001`), counter anti-cloning ditingkatkan tiap assertion sukses.
- **[DONE] TASK-29** — Worker CSV (gated RUM di-override oleh instruksi user). Server/Shared: `lib/workers/download.worker.ts` (message `{rows, delimiter}` → `{csv}`; BOM + double-quote escape; timeout 30 s). Integrated util: `lib/download.ts` → `buildCsvViaWorker(rows, delimiter)` (fallback sync bila Worker tidak tersedia) dan `exportCsvViaWorker(filename, rows, delimiter)`. Adopter contoh: `components/assets/AssetRegistry.tsx` → tombol Export kini `async` + worker + save-as. (Komponen export lain — InventoryLedger, PurchaseList, dsb — masih pola sinkron `downloadText`; dapat bermigrasi bertahap.)
- **[DONE] TASK-30** — Save-as picker (Chromium `showSaveFilePicker`). `lib/download.ts` → `saveAsViaPickerOrDownload(filename, blob, mimeType)`; fallback anchor-download di non-FSAccess. User cancel (AbortError) tidak dipaksa fallback. Integrated di AssetRegistry (export CSV kini picker-aware).

- **[DONE] tsc-debt cleanup (seluruh file)** — 0 error `npx tsc --noEmit`: memperbaiki WoRow import (inspection/pm), actorRole hilang (onboarding), queue/jobs RouteHandler generic, grn RouteMeta zod-nullish transform, NotificationDto `severity` field, LaborStopwatch alias prop, cn import (inventory detail), Badge variant='outline'→'hold' 2 locus, WoDetail status literals DRAFT/PENDING_DISPATCH → OPEN/DISPATCHED, WoDialogs PrintButton/ExportDiolog prop number optional, grn zod nullish, sw register mark('1')→mark(1).

- **Rotating Code Debt (saysal baru muncul di wave-5, belum / tidak akan di-tackle di wave ini):**
  - VendorEscalationInput.reason opsional (default '') → eskalasi tanpa P1 reason tidak memicu push (by design; reason/severity disimpan di log structured).
  - Passkey pending challenge maps in-memory (single-process dev) — produksi butuh Redis/DB + in-flight request TTL yang lebih ketat.
  - Migration drizzle `0001_sleepy_lilith.sql` memakai nama serial acak dari `drizzle-kit generate` (bukan nama deskriptif) bukan code-debt; cukup catat.
  - Worker CSV di-download seluruh rows ke main thread untuk query/filter? Tidak, semua operasi filter terjadi di komponen sebelum buildCsvViaWorker; Worker hanya serialisasi — longtask > 50 ms beresiko tetap pada rows > 100 k (gugurkan ke plan lanjutan).
